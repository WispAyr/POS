...
export function PaymentTrackingView() {
  ...
  const [view, setView] = useState<
    'status' | 'active' | 'statistics' | 'validate'
  >('status');
  ...
  const loadActivePayments = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/payment/active/${selectedSite}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const loadStatistics = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/payment/statistics/${selectedSite}`,
      );
      if (response.ok) {
        const data = await response.json();
        setStatistics(data);
  ...
  const loadExpiringPayments = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/payment/expiring/${selectedSite}?minutes=30`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const searchPaymentStatus = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/payment/status/${selectedSite}/${encodeURIComponent(searchVrm.toUpperCase().replace(/\s/g, ''))}`,
      );
      if (!response.ok) {
  ...
  const validatePayment = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/payment/validate/${selectedSite}/${encodeURIComponent(searchVrm.toUpperCase().replace(/\s/g, ''))}`,
      );
      if (!response.ok) {
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 dark:text-gray-400">
                          <div>
                            ...
                            {formatTimestamp(payment.startTime)}
                          </div>
                          <div>
                            <span className="font-medium">Expires:</span>{' '}
                            {formatTimestamp(payment.expiryTime)}
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            {getTimeRemaining(payment.expiryTime)}
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      {view === 'validate' && (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-800 transition-colors">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Check if a vehicle has valid payment for barrier control (real-time
            validation)
          </p>
          <div className="flex gap-4 mb-4">
            <input
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <span className="ml-2 text-gray-900 dark:text-white">
                        {formatCurrency(validationResult.payment.amount)}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700 dark:text-gray-300">
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-gray-900 dark:text-white">
                        {formatTimestamp(validationResult.expiresAt!)}
                      </span>
                    </div>
                    <div>
                      <span className="font-medium text-gray-700 dark:text-gray-300">
                      ...
                      <span className="ml-2 text-gray-900 dark:text-white">
                        {validationResult.remainingMinutes} minutes
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        Expires in {getTimeRemaining(payment.expiryTime)}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-800 transition-colors">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Avg: {formatCurrency(statistics.averageAmount)}
            </p>
          </div>
        </div>
      )}
