...
export function EnforcementReview() {
    ...
    // TODO: Uncomment when vehicle notes/markers UI is implemented
    ...
    const fetchQueue = async () => {
        try {
            ...
            const { data } = await axios.get(`/enforcement/queue?${params.toString()}`);
            setQueue(data);
    ...
    // TODO: Implement handleAddVehicleNote and handleAddMarker when UI is ready
    ...
    if (!currentDecision) {
        return (
            <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 min-h-[400px]">
                ...
                <p className="text-gray-500 dark:text-gray-400 mt-2">No pending violations to review.</p>
                <button
                    onClick={fetchQueue}
    ...
    const entryPlateImage = currentDecision.metadata?.entryImages?.find(img => img.type === 'plate')?.url;
    const entryOverviewImage = currentDecision.metadata?.entryImages?.find(img => img.type === 'overview')?.url;
    const exitPlateImage = currentDecision.metadata?.exitImages?.find(img => img.type === 'plate')?.url;
    const exitOverviewImage = currentDecision.metadata?.exitImages?.find(img => img.type === 'overview')?.url;
        ...
        <div className="flex gap-4 h-[calc(100vh-8rem)]">
                ...
                <div className="w-72 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden flex flex-col">
                    <div className="p-4 border-b border-gray-200 dark:border-slate-800">
                        <div className="flex justify-between items-center mb-3">
                            <div>
                                <h3 className="font-bold text-gray-900 dark:text-white">Queue</h3>
                                <p className="text-xs text-gray-500 dark:text-gray-400">{queue.length} pending</p>
                            </div>
                            <button
                        ...
                        {queue.map((decision) => (
                            <button
                                ...
                                className={`w-full p-3 text-left border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors ${
                                    currentDecision?.id === decision.id ? 'bg-blue-50 dark:bg-blue-900/20 border-l-4 border-l-blue-600' : ''
                                }`}
                            >
                                <div className="font-bold text-sm text-gray-900 dark:text-white mb-1">{decision.vrm}</div>
                                <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">
                                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${
                                        decision.reason === 'NO_VALID_PAYMENT' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' :
                                        decision.reason === 'OVERSTAY' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400' :
                                        'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400'
                                    }`}>
                                        {decision.reason.replace(/_/g, ' ')}
                                    </span>
                                </div>
                                <div className="text-xs text-gray-600 dark:text-gray-400">{decision.siteId} • {formatDuration(decision.durationMinutes)}</div>
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{currentDecision.vrm}</h2>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden mb-4">
                    <div className="grid grid-cols-4 gap-1 bg-gray-900 p-1">
                        <div className="relative group overflow-hidden aspect-video">
                            {entryOverviewImage ? (
                                <img src={entryOverviewImage} alt="Entry Overview" className="w-full h-full object-cover cursor-pointer" onClick={() => window.open(entryOverviewImage, '_blank')} />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-gray-500">No Image</div>
                            )}
                            <div className="absolute bottom-1 left-1 bg-green-600/90 text-white text-xs px-2 py-0.5 rounded font-semibold">Entry Overview</div>
                        </div>
                        <div className="relative group overflow-hidden aspect-video">
                            {entryPlateImage ? (
                                <img src={entryPlateImage} alt="Entry Plate" className="w-full h-full object-cover cursor-pointer" onClick={() => window.open(entryPlateImage, '_blank')} />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-gray-500 bg-gray-800"><Car className="w-8 h-8 text-gray-600" /></div>
                            )}
                            <div className="absolute bottom-1 left-1 bg-green-600/90 text-white text-xs px-2 py-0.5 rounded font-semibold">Entry Plate</div>
                        </div>
                        <div className="relative group overflow-hidden aspect-video">
                            {exitOverviewImage ? (
                                <img src={exitOverviewImage} alt="Exit Overview" className="w-full h-full object-cover cursor-pointer" onClick={() => window.open(exitOverviewImage, '_blank')} />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-gray-500">No Image</div>
                            )}
                            <div className="absolute bottom-1 left-1 bg-red-600/90 text-white text-xs px-2 py-0.5 rounded font-semibold">Exit Overview</div>
                        </div>
                        <div className="relative group overflow-hidden aspect-video">
                            {exitPlateImage ? (
                                <img src={exitPlateImage} alt="Exit Plate" className="w-full h-full object-cover cursor-pointer" onClick={() => window.open(exitPlateImage, '_blank')} />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-gray-500 bg-gray-800"><Car className="w-8 h-8 text-gray-600" /></div>
                            )}
                            <div className="absolute bottom-1 left-1 bg-red-600/90 text-white text-xs px-2 py-0.5 rounded font-semibold">Exit Plate</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-200 dark:border-slate-800 p-4 mb-4 overflow-y-auto flex-1">
                        <div className="grid grid-cols-3 gap-4 mb-4">
                            ...
                            <div className="bg-gray-50 dark:bg-slate-800 p-3 rounded-lg">
                                ...
                                <span className="text-sm font-bold text-gray-900 dark:text-white">{(currentDecision.confidenceScore * 100).toFixed(0)}%</span>
                            </div>
                        </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-gray-200 dark:border-slate-800 p-4">
                    <div className="mb-3">
                        <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Decision Notes (Optional)</label>
                        <textarea
                            value={decisionNote}
