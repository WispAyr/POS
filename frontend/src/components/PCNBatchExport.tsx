        ...
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              {pcns.length} PCN(s) ready for export • {selectedPcns.size}{' '}
              selected
            </p>
          </div>
          <button
          ...
          >
            ...
            {exporting
              ? 'Exporting...'
              : `Export ${selectedPcns.size || ''} PCN(s)`}
          </button>
        </div>
                ...
                {pcns.map((pcn) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                      {pcn.entryTime
                        ? new Date(pcn.entryTime).toLocaleString()
                        : 'N/A'}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                      {pcn.exitTime
                        ? new Date(pcn.exitTime).toLocaleString()
                        : 'N/A'}
                    </td>
                    <td className="px-6 py-4">
                      <button
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-2 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${viewingPcn.confidenceScore * 100}%`,
                          }}
                        />
                      </div>
                      <span className="font-bold text-gray-900 dark:text-white">
                        {(viewingPcn.confidenceScore * 100).toFixed(0)}%
                      </span>
                    </div>
                  </div>
                  <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-lg">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white text-lg">
                      {formatDuration(viewingPcn.durationMinutes)}
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-lg">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">
                      {viewingPcn.entryTime
                        ? new Date(viewingPcn.entryTime).toLocaleString()
                        : 'N/A'}
                    </span>
                  </div>
                  <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-lg">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">
                      {viewingPcn.exitTime
                        ? new Date(viewingPcn.exitTime).toLocaleString()
                        : 'N/A'}
                    </span>
                  </div>
                </div>
                    ...
                    {auditLogs.map((log) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span
                              className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getActionColor(log.action)}`}
                            >
                              {log.action}
                            </span>
