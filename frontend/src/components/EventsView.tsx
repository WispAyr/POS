...
export function EventsView() {
  ...
  const fetchEvents = useCallback(
    async (page = 1) => {
      ...
      try {
        const hideUnknown =
          localStorage.getItem('hideUnknownPlates') === 'true';
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="bg-gray-100 dark:bg-slate-900 rounded-xl h-64 animate-pulse border border-gray-200 dark:border-slate-800"
            />
          ))}
        </div>
      ) : events.length === 0 ? (
