...
import { Movement, Session, Decision, Payment, Permit } from '../domain/entities';
...
export class AuditService {
    ...
    async log(context: AuditContext): Promise<AuditLog> {
        ...
        this.logger.debug(`Audit logged: ${context.action} for ${context.entityType}:${context.entityId}`);
        return saved;
    }
     ...
     */
    async logMovementIngestion(movement: Movement, isNew: boolean, parentAuditId?: string): Promise<AuditLog> {
        return this.log({
            entityType: 'MOVEMENT',
            ...
            details: {
                ...
                images: movement.images?.map(img => ({
                    url: img.url,
                    type: img.type,
                    timestamp: img.timestamp,
                })) || [],
                source: movement.rawData?.source || movement.rawData?.cameraType || 'UNKNOWN',
     ...
     */
    async logSessionCreation(session: Session, entryMovement: Movement, entryAuditId?: string): Promise<AuditLog> {
        return this.log({
            entityType: 'SESSION',
            entityId: session.id,
            ...
            relatedEntities: [
                { entityType: 'MOVEMENT', entityId: entryMovement.id, relationship: 'CREATED_BY' },
    ...
    async logSessionCompletion(
        ...
        return this.log({
            ...
            relatedEntities: [
                { entityType: 'MOVEMENT', entityId: exitMovement.id, relationship: 'COMPLETED_BY' },
            ],
            siteId: session.siteId,
            vrm: session.vrm,
     ...
     */
    async logDecisionCreation(decision: Decision, session: Session, sessionCompletedAuditId?: string): Promise<AuditLog> {
        return this.log({
            entityType: 'DECISION',
    ...
    async logPaymentIngestion(payment: Payment): Promise<AuditLog> {
        return this.log({
            ...
            relatedEntities: [
                { entityType: 'SITE', entityId: payment.siteId, relationship: 'OCCURRED_AT' },
            ],
            siteId: payment.siteId,
            vrm: payment.vrm,
            metadata: {
    ...
    async logPermitIngestion(permit: Permit): Promise<AuditLog> {
        return this.log({
            ...
            relatedEntities: permit.siteId
                ? [{ entityType: 'SITE', entityId: permit.siteId, relationship: 'APPLIES_TO' }]
                : [],
    ...
    async logReconciliationTrigger(
        ...
        return this.log({
            ...
            relatedEntities: [
                { entityType: triggerType, entityId: triggerEntityId, relationship: 'TRIGGERS' },
            ],
            siteId,
            vrm,
        });
    ...
    async logDecisionReconciliation(
        ...
        return this.log({
            ...
            relatedEntities: [
                { entityType: triggerType, entityId: triggerEntityId, relationship: 'TRIGGERED_BY' },
    ...
    async getAuditTrailByVrm(vrm: string, options?: AuditQueryOptions): Promise<AuditLog[]> {
        ...
        if (options?.actionFilter && options.actionFilter.length > 0) {
            query.andWhere('audit.action IN (:...actions)', { actions: options.actionFilter });
        }
    ...
    async getTimeline(vrm: string, startDate?: Date, endDate?: Date): Promise<AuditTimeline> {
        ...
        // Get all audit logs
        const auditLogs = await this.getAuditTrailByVrm(normalizedVrm, { startDate, endDate });
        ...
        const [movements, sessions, decisions, payments, permits] = await Promise.all([
            movementIds.size > 0 ? this.movementRepo.findBy({ id: In(Array.from(movementIds)) }) : [],
            sessionIds.size > 0 ? this.sessionRepo.findBy({ id: In(Array.from(sessionIds)) }) : [],
            decisionIds.size > 0 ? this.decisionRepo.findBy({ id: In(Array.from(decisionIds)) }) : [],
            paymentIds.size > 0 ? this.paymentRepo.findBy({ id: In(Array.from(paymentIds)) }) : [],
            permitIds.size > 0 ? this.permitRepo.findBy({ id: In(Array.from(permitIds)) }) : [],
        ]);
    ...
    async getEnforcementCaseHistory(decisionId: string): Promise<EnforcementCaseHistory> {
        ...
        const session = await this.sessionRepo.findOne({ where: { id: decision.sessionId } });
        if (!session) {
        ...
        const [payments, permits] = await Promise.all([
            this.paymentRepo.find({ where: { vrm: session.vrm, siteId: session.siteId } }),
        ...
        const timeline = auditTrail
            .filter(log =>
                log.entityType === 'MOVEMENT' && (log.entityId === session.entryMovementId || log.entityId === session.exitMovementId) ||
    ...
    async searchAuditLogs(query: {
        ...
        if (query.vrm) {
            qb.andWhere('audit.vrm = :vrm', { vrm: query.vrm.toUpperCase().replace(/\s/g, '') });
        }
        ...
        if (query.startDate) {
            qb.andWhere('audit.timestamp >= :startDate', { startDate: query.startDate });
        }
