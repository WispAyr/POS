...
export class SessionService {
    ...
    async processMovement(movement: Movement): Promise<void> {
        ...
        if (movement.direction === 'ENTRY') {
            ...
            this.logger.log(`Movement ${movement.id} direction ${movement.direction} ignored`);
        }
    }
    ...
    private async handleExit(movement: Movement) {
        ...
        if (openSession) {
            ...
            openSession.durationMinutes = Math.floor((movement.timestamp.getTime() - openSession.startTime.getTime()) / 60000);
            ...
            await this.ruleEngine.evaluateSession(savedSession).catch(err => {
                this.logger.error(`Error evaluating session ${savedSession.id}`, err.stack);
            });
