...
export class ReconciliationService {
    ...
    async reconcilePayment(vrm: string, siteId: string, paymentStartTime: Date, paymentExpiryTime: Date): Promise<{
        ...
        this.logger.log(`Found ${overlappingSessions.length} sessions to re-evaluate`);
        ...
        for (const session of overlappingSessions) {
            ...
            if (existingDecision && existingDecision.outcome !== newDecision.outcome) {
                this.logger.log(
     ...
     */
    async reconcilePermit(vrm: string, siteId: string | null, permitActive: boolean): Promise<{
        sessionsReevaluated: number;
        decisionsUpdated: number;
    }> {
        this.logger.log(`Reconciling sessions for VRM ${vrm} with permit (site: ${siteId}, active: ${permitActive})`);
     ...
     */
    async reconcileSite(siteId: string, limit: number = 100): Promise<{
        sessionsReevaluated: number;
        decisionsUpdated: number;
    }> {
