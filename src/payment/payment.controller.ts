...
export class PaymentController {
  ...
  async validatePayment(
    ...
    return this.paymentTrackingService.validatePaymentForAccess(
      vrm,
      siteId,
      checkTime,
    );
  ...
  async paymentWebhook(@Body() payload: any) {
    ...
    const dto: IngestPaymentDto = {
      ...
      externalReference:
        payload.reference || payload.transactionId || payload.externalReference,
    };
  ...
  async getExpiringPayments(
    ...
    return this.paymentTrackingService.getPaymentsExpiringSoon(
      siteId,
      minutesNum,
    );
  }
  ...
  async isPaymentMachineEnabled(@Param('siteId') siteId: string) {
    return {
      enabled:
        await this.paymentTrackingService.isPaymentMachineEnabled(siteId),
    };
  }
}
