...
export class PaymentTrackingService {
    ...
    async validatePaymentForAccess(
        ...
        await this.auditService.log({
            ...
            relatedEntities: [
                { entityType: 'PAYMENT', entityId: bestPayment.id, relationship: 'VALIDATES' },
            ],
            siteId,
            vrm: normalizedVrm,
        });
    ...
    async getPaymentStatus(vrm: string, siteId: string): Promise<PaymentStatus> {
        ...
        const nextExpiry = futurePayments.length > 0
            ? futurePayments.sort((a, b) => a.expiryTime.getTime() - b.expiryTime.getTime())[0].expiryTime
            : undefined;
     ...
     */
    async getPaymentStatistics(siteId: string, startDate?: Date, endDate?: Date): Promise<{
        totalPayments: number;
        ...
        const averageAmount = allPayments.length > 0 ? totalRevenue / allPayments.length : 0;
     ...
     */
    async getPaymentsExpiringSoon(siteId: string, minutes: number = 30): Promise<Payment[]> {
        const now = new Date();
