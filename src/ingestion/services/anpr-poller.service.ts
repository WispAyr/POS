    ...
    async pollEvents(hours?: number, limit?: number, offset?: number): Promise<{ processed: number; new: number; updated: number; errors: number }> {
        ...
        try {
            ...
            const response = await firstValueFrom(
                ...
                this.logger.error(`External API call failed: ${err.message}`, err.stack);
                throw err;
            });
                    ...
                    await Promise.all(batch.map(async (event) => {
                        ...
                        if (event.images && typeof event.images === 'object') {
                            if (event.images.plate) {
                                const filename = await this.imageService.downloadAndStore(event.images.plate, 'plate').catch(() => null);
                                if (filename) dto.images.push({ url: `/api/images/${filename}`, type: 'plate' });
                            }
                            if (event.images.overview) {
                                const filename = await this.imageService.downloadAndStore(event.images.overview, 'overview').catch(() => null);
                                if (filename) dto.images.push({ url: `/api/images/${filename}`, type: 'overview' });
                            }
                        }
                        ...
                        try {
                            ...
                            errorCount++;
                            this.logger.error(`Failed to ingest event for ${dto.plateNumber} at site ${dto.siteId}: ${err.message}`);
                        }
                    }));
                }

                this.logger.log(`Poll complete: ${ingestedNew} new, ${ingestedUpdated} updated, ${errorCount} errors.`);
                return {
