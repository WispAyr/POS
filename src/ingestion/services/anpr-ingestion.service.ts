...
export class AnprIngestionService {
  ...
  async ingest(
    dto: IngestAnprDto,
  ): Promise<{ movement: Movement; isNew: boolean }> {
    ...
    if (!vrmRaw) {
      this.logger.warn(
        `Missing VRM/plateNumber in payload: ${JSON.stringify(dto)}`,
      );
    ...
    if (existing) {
      ...
      const hasRemoteImages =
        existing.images?.some((img) => img.url.includes('localhost:3000')) ||
        !existing.images?.length;
      const hasLocalImagesInDto = dto.images?.some((img) =>
        img.url.startsWith('/api/images/'),
      );

      if (hasRemoteImages && hasLocalImagesInDto) {
        this.logger.log(
          `Updating images for existing movement: ${existing.id} (${vrm})`,
        );
