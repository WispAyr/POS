...
import {
  ...
  MondayItem,
  MondayColumnValue,
} from './interfaces/monday-data.interface';
...
export class MondayIntegrationService {
    ...
    try {
      ...
      while (hasMore && cursor) {
        ...
        if (nextResponse.data.errors) {
          this.logger.error(
            'GraphQL Errors during pagination',
            nextResponse.data.errors,
          );
          break;
        }
  ...
  private async syncSites() {
    ...
    for (const item of items) {
      const siteIdCol = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'text_mkt4k6yt',
      );
      const statusCol = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'color_mkpjp7nb',
      );
  ...
  async syncWhitelists() {
    ...
    for (const item of items) {
      ...
      let siteId = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'text_mkr3e6as',
      )?.text;
      ...
      const startDateStr = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'date_mkpj4ap1',
      )?.text;
      const endDateStr = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'date_mkqeq1q6',
      )?.text;
      ...
      if (vrm) {
        ...
        this.logger.debug(
          `Synced Permit from Monday: ${vrm} (Item ID: ${item.id})`,
        );
        count++;
      }
    }
    ...
    for (const local of localPermits) {
      if (local.mondayItemId && !currentMondayIds.has(local.mondayItemId)) {
        this.logger.log(
          `Deleting local permit ${local.vrm} as it was removed from Monday (ID: ${local.mondayItemId})`,
        );
  ...
  async pushPermitToMonday(permit: Permit) {
    ...
    const columnValues = JSON.stringify({
      ...
      date_mkqeq1q6: permit.endDate
        ? permit.endDate.toISOString().split('T')[0]
        : null,
    });
    ...
    try {
      ...
      if (response.data.data?.create_item?.id) {
        ...
        await this.permitRepo.save(permit);
        this.logger.log(
          `Pushed permit ${permit.vrm} to Monday (ID: ${permit.mondayItemId})`,
        );
      }
    } catch (error) {
  ...
  async updatePermitOnMonday(permit: Permit) {
    ...
    try {
      ...
      this.logger.log(
        `Updated permit ${permit.vrm} on Monday (ID: ${permit.mondayItemId})`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to update permit ${permit.vrm} on Monday`,
        error,
      );
    }
  }
  ...
  async deletePermitFromMonday(mondayItemId: string) {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to delete permit ${mondayItemId} from Monday`,
        error,
      );
    }
  }
  ...
  async getOrCreateCameraConfigBoard(): Promise<number> {
    ...
    try {
      ...
      if (response.data.errors) {
        this.logger.error(
          'Failed to create Camera Config board',
          response.data.errors,
        );
        throw new Error('Failed to create board');
      }

      this.cameraConfigBoardId = parseInt(
        response.data.data.create_board.id,
        10,
      );
      this.logger.log(
        `Created Camera Config Board: ${this.cameraConfigBoardId}`,
      );
  ...
  async pushCameraToMonday(
    siteId: string,
    cameraId: string,
    towardsDir: string = 'ENTRY',
    awayDir: string = 'EXIT',
  ) {
    ...
    try {
      ...
      if (response.data.errors) {
        this.logger.error(
          `Failed to push camera ${cameraId}`,
          response.data.errors,
        );
        return null;
      }
  ...
  async syncCameraConfigs() {
    ...
    const boardId =
      this.configService.get<string>('MONDAY_CAMERA_BOARD_ID') || '1952030503';
    ...
    for (const item of items) {
      ...
      const siteIdCol = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'text_mkqv97ry',
      );
      const towardsDirCol = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'text_mkw217pv',
      );
      const awayDirCol = item.column_values.find(
        (c: MondayColumnValue) => c.id === 'text_mkw2sxg9',
      );
      ...
      if (siteId && cameraId && cameraId !== '?') {
        ...
        if (site) {
          ...
          let camConfig = site.config.cameras.find(
          ...
          camConfig.towardsDirection = towardsDirection;
          camConfig.awayDirection = awayDirection;

          await this.siteRepo.save(site);
          this.logger.debug(
            `Synced camera config: ${cameraId} (${siteId}) - Towards=${towardsDirection}, Away=${awayDirection}`,
          );
        } else {
    ...
    this.logger.log(
      `Camera config sync complete. Processed ${items.length} items.`,
    );
