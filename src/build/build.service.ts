...
export class BuildService {
    ...
    async getBuildMetadata(): Promise<BuildMetadata> {
        ...
        try {
            ...
            metadata.npmVersion = execSync('npm --version', { encoding: 'utf-8' }).trim();
    ...
    async logBuildComplete(
        ...
        buildAudit.duration = options?.duration || (buildAudit.completedAt.getTime() - buildAudit.timestamp.getTime());
    ...
    async getBuildHistory(options?: {
        ...
        if (options?.buildType) {
            query.andWhere('build.buildType = :buildType', { buildType: options.buildType });
        }
        ...
        if (options?.startDate) {
            query.andWhere('build.timestamp >= :startDate', { startDate: options.startDate });
        }

        if (options?.endDate) {
            query.andWhere('build.timestamp <= :endDate', { endDate: options.endDate });
        }
    ...
    private getGitTag(): string | undefined {
        try {
            return execSync('git describe --tags --exact-match 2>/dev/null || echo ""', { encoding: 'utf-8' }).trim() || undefined;
        } catch {
            return undefined;
        }
    }
